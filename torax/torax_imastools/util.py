...
def save_netCDF(
    ...
    if file_name is None:
        file_name = "IDS_file_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
...
@requires_module("imaspy")
def load_IMAS_equilibrium_from_Data_entry(file_path: str) -> IDSToplevel:
    """Loads the equilibrium IDS for a single time slice from a specific data
...
@requires_module("imaspy")
def load_IMAS_data_from_netCDF(file_path: str) -> IDSToplevel:
    """Loads the equilibrium IDS for a single time slice from an IMAS netCDF
    file path"""
...
@requires_module("imaspy")
def load_IMAS_data_from_hdf5(directory_path: str) -> IDSToplevel:
    """Loads the equilibrium IDS for a single time slice from the path of a
...
@requires_module("imaspy")
def load_IMAS_data(path: str) -> IDSToplevel:
    """Loads the equilibrium IDS for a single time slice either from a netCDF
